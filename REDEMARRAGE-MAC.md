...
Ouvre une seule fois `installer-demarrage-automatique.command` par double-clic.

Flight Director démarrera ensuite automatiquement à chaque ouverture de session du Mac, sans fenêtre Terminal. L’adresse demeure :

```text
http://localhost:3000
```

Pour désactiver ce fonctionnement, ouvre `desinstaller-demarrage-automatique.command`.
