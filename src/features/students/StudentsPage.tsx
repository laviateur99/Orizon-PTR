...
import { subscribeAllProgressActivities } from "@/features/training-progress/firestore";
import type { ProgressActivity } from "@/features/training-progress/types";
...
export function StudentsPage(){
  ...
  const flightHoursByStudent=useMemo(()=>{
    ...
    activities.filter(item=>item.status==="Complété"&&(item.type==="Double commande"||item.type==="Solo")).forEach(item=>{
      const hours=item.hobbsStart!==undefined&&item.hobbsEnd!==undefined?Math.max(0,item.hobbsEnd-item.hobbsStart):(item.airtimeMinutes||0)/60;
      totals[item.studentId]=(totals[item.studentId]||0)+hours;
    });
    return totals;
  },[activities]);
  useEffect(()=>{
  ...
  const filtered=useMemo(()=>students.filter(s=>{
    ...
    if(sortBy==="hours")return (flightHoursByStudent[b.id]||0)-(flightHoursByStudent[a.id]||0);
    return nameA.localeCompare(nameB);
  }),[students,search,status,sortBy,flightHoursByStudent]);
  return <>
