...
export type TrainingQuoteLine={id:string;rateId:string;description:string;quantity:number;unit:TrainingRateUnit;unitPrice:number;subtotal:number;taxable:boolean};
export type TrainingTaxSettings={gstRate:number;qstRate:number;updatedAt?:unknown};
export type TrainingQuote={id:string;quoteNumber:string;date:string;expirationDate:string;customerType:"student"|"prospect";studentId?:string;customerName:string;customerPhone:string;customerEmail:string;trainingType:string;notes:string;status:TrainingQuoteStatus;lines:TrainingQuoteLine[];subtotal:number;gstRate:number;qstRate:number;gst:number;qst:number;taxes:number;total:number;templateId?:string;createdBy:string;createdAt?:unknown;updatedAt?:unknown};
// Structure réservée aux modèles futurs; aucune interface de gestion n'est activée pour le moment.
export type TrainingQuoteTemplate={id:string;name:string;trainingType:string;active:boolean;lines:Array<{rateId:string;quantity:number;description?:string}>;createdAt?:unknown;updatedAt?:unknown};
