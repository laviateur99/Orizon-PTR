...
import { initializeAircraftMaintenanceSchedule,saveAircraft } from "./firestore";
import type { Aircraft,MaintenanceTask } from "./types";
import maintenanceSeed from "@/features/maintenance/maintenance-seed.json";
import {useAuth} from "@/features/auth/AuthProvider";

type CalendarChoice="C152"|"C172"|"PA31"|"Aucun";
const calendarTemplateAircraft:Record<Exclude<CalendarChoice,"Aucun">,string>={C152:"C-FZSX",C172:"C-GOAQ",PA31:"C-GUMQ"};
...
export function ResourceManagement({
  ...
  const [calendarDraft,setCalendarDraft]=useState<CalendarChoice>("C152");
  ...
  async function saveAircraftDraft() {
    ...
    if((aircraftDraft.hobbsTotal||0)<0||(aircraftDraft.airTimeTotal||0)<0){setMessage("Le Hobbs et l’Air Time doivent être positifs.");return}
    const isNew=aircraftDraft.id.startsWith("AIRCRAFT-");
    const saved={
      ...aircraftDraft,
      id: registration,
      registration
    };
    await saveAircraft(saved);
    if(isNew&&calendarDraft!=="Aucun"){
      const sourceAircraftId=calendarTemplateAircraft[calendarDraft];
      const templates=(maintenanceSeed.tasks as MaintenanceTask[]).filter(item=>item.aircraftId===sourceAircraftId);
      await initializeAircraftMaintenanceSchedule(saved,calendarDraft,templates,{id:user?.uid||"",name:profile?.name||profile?.email||"",role:profile?.role||""});
    }
    setAircraftDraft(null);
    setMessage(isNew&&calendarDraft!=="Aucun"?`Avion enregistré avec le calendrier ${calendarDraft}. Configurez maintenant les dernières exécutions dans Maintenance.`:"Avion enregistré.");
  }
    ...
    <section className="card resource-management">
      <header className="resource-management-head">
        ...
        <div className="resource-add-actions">
          <button
            className="button"
            onClick={() => {setCalendarDraft("C152");setAircraftDraft(newAircraftTemplate())}}
          >
            Ajouter un avion
          </button>
          <button
          ...
          <section className="modal compact">
            ...
            <div className="modal-body">
              ...
              <div className="form-grid">
                <label>
                  Hobbs actuel
                  <input type="number" min="0" step="0.1" value={aircraftDraft.hobbsTotal??0} onChange={event=>setAircraftDraft({...aircraftDraft,hobbsTotal:Number(event.target.value)})}/>
                </label>
                <label>
                  Air Time actuel
                  <input type="number" min="0" step="0.1" value={aircraftDraft.airTimeTotal??0} onChange={event=>setAircraftDraft({...aircraftDraft,airTimeTotal:Number(event.target.value)})}/>
                </label>
              </div>
              {aircraftDraft.id.startsWith("AIRCRAFT-")&&<label>
                Calendrier de maintenance applicable
                <select value={calendarDraft} onChange={event=>setCalendarDraft(event.target.value as CalendarChoice)}><option>C152</option><option>C172</option><option>PA31</option><option>Aucun</option></select>
                <small>Les items seront créés sans échéance initiale. Les dernières exécutions, heures et dates devront être saisies dans Maintenance.</small>
              </label>}
            </div>
            <footer>
              <span />
