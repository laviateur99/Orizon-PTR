...
const statusClass = (status: AircraftStatus) =>
  status === "Disponible" ? "ok" : status === "Maintenance planifiée" || status === "Inspection" ? "warn" : "danger";

export function FleetPage() {
  const [aircraft, setAircraft] = useState<Aircraft[]>([]);
  const [snags, setSnags] = useState<Snag[]>([]);
  const [snagFilter, setSnagFilter] = useState("");
  const displayedSnags = snags.filter(snag => !snagFilter || snag.aircraftId === snagFilter);
  ...
  return (
    <>
            ...
            {group.items.map(item => {
              ...
              <article className="aircraft-card" key={item.id}>
                ...
                <div className="aircraft-actions">
                  ...
                  <a className="button secondary" href="#fleet-snags" onClick={()=>setSnagFilter(item.id)}>Consulter les SNAG ({snags.filter(snag=>snag.aircraftId===item.id).length})</a>
      ...
      <section className="card" id="fleet-snags">
        <h2>SNAG et défectuosités</h2>
        <label>Avion<select value={snagFilter} onChange={event=>setSnagFilter(event.target.value)}><option value="">Tous les avions</option>{aircraft.map(item=><option key={item.id} value={item.id}>{item.registration}</option>)}</select></label>
        <div className="snag-list">
          {displayedSnags.map(snag => (
            <article className="snag-row" key={snag.id}>
              ...
              <div className="snag-status">
                <select disabled={Boolean(snag.workOrderId) || snag.status === "Fermé"} value={snag.status} onChange={event => updateSnag(snag.id, { status: event.target.value as SnagStatus })}>
                ...
                {snag.status !== "Fermé" && (
                  <a
                    ...
                    href={
                      snag.workOrderId
                        ? `/maintenance?workOrderId=${encodeURIComponent(snag.workOrderId)}`
                        : `/maintenance?snagId=${encodeURIComponent(snag.id)}`
                    }
                  >
                    {snag.workOrderId ? "Ouvrir le travail" : "Créer le travail"}
                  </a>
                )}
              </div>
            </article>
          ))}
          {!displayedSnags.length && <p>Aucun SNAG pour cet avion.</p>}
        </div>
      </section>
      ...
      {editing && (
        <div className="modal-backdrop"><section className="modal compact">
          ...
          <div className="modal-body">
            <label>Statut<select value={editing.status} onChange={event => setEditing({ ...editing, status: event.target.value as AircraftStatus })}><option>Disponible</option><option>Maintenance planifiée</option><option>En maintenance</option><option>Retour en service retardé</option><option>Hors service</option><option>Inspection</option><option>SNAG</option></select></label>
          ...
          <div className="modal-body">
            ...
            <div className="impacted-flight-list">{impactedReservations.map(reservation=>{const value=impactActions[reservation.id];const available=aircraft.filter(item=>item.active&&item.status==="Disponible"&&item.id!==impactSnag.aircraft.id);return <article className={`impacted-flight ${value?.completed?"completed":""}`} key={reservation.id}>
              ...
              <div className="impact-controls">
                <label>Action<select disabled={value?.completed} value={value?.action||"À décider plus tard"} onChange={e=>setImpactActions(current=>({...current,[reservation.id]:{...current[reservation.id],action:e.target.value as ImpactResolutionAction}}))}><option>Déplacer vers un autre avion</option><option>Aviser l’élève</option><option>Aviser l’instructeur</option><option>Aviser l’élève et l’instructeur</option><option>Annuler le vol</option><option>Reporter le vol</option><option>À décider plus tard</option></select></label>
