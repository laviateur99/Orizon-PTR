...
import{MaintenanceSectionNav}from"@/features/maintenance/MaintenanceSectionNav";
import {
  deleteSnagAsAdmin,
...
export function SnagDashboardPage() {
  ...
  const [status, setStatus] = useState<SnagStatus | "Actifs" | "Tous">("Actifs");
  ...
  return (
    <>
      ...
      <MaintenanceSectionNav active="snags"/>
      ...
      <div className="snag-dashboard-toolbar">
        ...
        <select value={status} onChange={event => setStatus(event.target.value as SnagStatus | "Actifs" | "Tous")}>
      ...
      <section className="card snag-table-card">
        ...
        {filtered.map(item => (
          <article className="snag-table-row" key={item.id}>
            ...
            <select disabled={Boolean(item.workOrderId) || item.status === "Fermé"} value={item.status} onChange={async event => {
              try {
                await updateSnag(item.id, { status: event.target.value as SnagStatus });
              } catch (value) {
                setError(value instanceof Error ? value.message : "Impossible de modifier le SNAG.");
              }
            }}>{STATUSES.map(value => <option disabled={value === "Fermé" && item.status !== "Fermé"} key={value}>{value}</option>)}</select>
            <span>{item.estimatedReturnDate || "—"}</span>
            <div className="snag-work-actions">
              <a className="button small" href={item.workOrderId ? `/maintenance?workOrderId=${encodeURIComponent(item.workOrderId)}` : `/maintenance?snagId=${encodeURIComponent(item.id)}`}>
                {item.workOrderId ? "Ouvrir le travail" : "Créer un travail"}
              </a>
              {!item.workOrderId && <button className="button danger small" onClick={()=>setDeleteTarget(item)}>Supprimer</button>}
            </div>
          </article>
        ))}
